...
1. **2018 monthly Ozone levels** ........................................... 4  
2. **2019 monthly Ozone levels** ........................................... 4
...
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_PM2.5 = round(mean(PM2.5),2)) %>%  #rounded to 2 digits
  arrange(mean_PM2.5)
...
```{r ozone2018, echo=FALSE, warning=FALSE, fig.align='left', fig.height=4, fig.width=6, fig.cap=" 2018 Monthly Ozone concentrations"}
# Filter data for 2018 and create the boxplot
epa_data_2018 <- epa_data %>% filter(year(Date) == 2018)

ggplot(epa_data_2018, aes(x = factor(month(Date, label=TRUE)), y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone concentrations (2018)", x = "Month", y = "Concentration (µg/m³)")
```
```{r ozone2019, echo=FALSE, warning=FALSE, fig.align='left', fig.height=4, fig.width=6, fig.cap="2019 Monthly Ozone concentrations"}
# Filter data for 2019 and create the boxplot
epa_data_2019 <- epa_data %>% filter(year(Date) == 2019)

ggplot(epa_data_2019, aes(x = factor(month(Date, label=TRUE)), y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone concentrations (2018)", x = "Month", y = "Concentration (µg/m³)")
...
## Git Repository
https://github.com/Marina777D/EDE_Fall2024
