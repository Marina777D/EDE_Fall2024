...
# Ensure the 'month' column is treated as a factor
peter_data$month <- factor(peter_data$month, levels = 1:12, labels = month.name[1:12])
...
plot_tp <- ggplot(peter_data, aes(x = month, y = tp_ug, fill = lakename)) +
  geom_boxplot() +
  labs(title = "Total Phosphorus by Month", x = "Month", y = "Total Phosphorus (ug/L)") +
  theme(legend.position = "none", axis.title.x = element_blank())  # Remove x-axis label and legend
